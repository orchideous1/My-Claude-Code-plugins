...
该脚本会创建 `.claude/state/sessions/<id>/` 并确保 `session.md` 与 `architecture.md` 的 frontmatter 完整。派生的 SESSION_ID 必须写入 `session.md` 的 `context.session_id` 字段，并在后续调用 `infer-workflow.sh`、`write-archive.sh`、`cleanup-session.sh`、`reset-session.sh` 等脚本时显式作为参数或环境变量传递，不再落盘到任何共享文件。
